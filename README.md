...
2. **Session distribution** — All pending keyword sessions are distributed round-robin across assigned devices. Each device runs its sessions sequentially; all devices run in parallel.
...
├── setup_devices.py          # Install Portal APK + assign healthy devices
├── check_devices.py          # Check USB debugging (Security Settings) status
├── check_devices.sh          # Same check as shell script
├── launch_scrcpy.sh          # Open scrcpy for assigned devices only
├── active_devices.json       # Healthy devices to use (auto-generated by --assign)
...
- DeepSeek API key
- Android devices connected via wireless ADB (WiFi)
...
### Step 3 — Install DroidRun Portal APK
...
### Step 4 — Assign healthy devices

Run a full health check (online + USB injection + Portal) and write only the passing devices to `active_devices.json`. Both `main.py` and `launch_scrcpy.sh` use this file automatically.
...
### Step 5 — View assigned devices on screen (optional)

Open a tiled scrcpy window for all assigned healthy devices (4-column grid, 320×568px per window):

```bash
bash launch_scrcpy.sh
```

If `active_devices.json` exists, only those devices are shown. Falls back to all connected devices if the file is missing. Press `Ctrl+C` to close all windows.
...
Edit the `_ALL_DEVICES` dict in `main.py` and the `DEVICE_POOL` dict in `setup_devices.py`:
...
After adding, run `python setup_devices.py --assign` to re-generate `active_devices.json`.
...
This will:
1. Load `active_devices.json` (or all devices if file is missing)
2. Load all clients and pending keywords (not yet run today)
3. Distribute sessions round-robin across assigned devices
4. Generate a DeepSeek prompt + optional follow-up for each session
5. Run all device queues in parallel (each device's sessions run sequentially)
6. Log results to `sessions_log.json`
...
Clears today's rotation so all keywords are treated as pending again.
...
# 5. (Optional) View assigned devices on screen
bash launch_scrcpy.sh &
...
| `python main.py --status` | Show today's session status |
| `python main.py --reset` | Reset today's rotation (re-run everything) |
| `python main.py --audit` | Run weekly ranking audit |
| `python setup_devices.py` | Check Portal status + auto-install where needed |
| `python setup_devices.py --check` | Check Portal status only (no install) |
| `python setup_devices.py --assign` | Full health check → write `active_devices.json` |
| `python check_devices.py` | Check USB debugging (Security Settings) on all devices |
| `bash check_devices.sh` | Same as above (shell version) |
| `bash launch_scrcpy.sh` | Open scrcpy for all assigned healthy devices |
...
| `logging.save_trajectory` | false | Save step-by-step screenshots (disabled to save disk space) |
...
| `active_devices.json` | Healthy devices assigned for sessions (generated by `--assign`) |
...
Increase `max_steps` in `config/config.yaml` (currently 80). Also check that the device has a stable WiFi connection — Chrome data is cleared automatically before each session to avoid CAPTCHAs.

### `sessions_log.json` stays empty

The log is at the **project root**: `sessions_log.json`. Make sure you are not looking at the old `logs/sessions_log.json` which is no longer used.

### Device not showing in scrcpy

Run `python setup_devices.py --assign` first to regenerate `active_devices.json`, then re-run `bash launch_scrcpy.sh`.
